...
function buildFocusEvidenceTask(params: {
  ...
  return [
    ...
    "- Include concrete decisions, file paths, command names, issue IDs, summary IDs, constraints, rejected options, unresolved questions, and handoff details when those details appear in Lossless summaries or direct expansions.",
    ...
    "- Keep this phase bounded. For lightweight targets, a concise evidence dossier from the active summaries is better than an exhaustive filesystem or repository audit.",
    "",
    "Tool boundary:",
    "- Use only Lossless recall tools for retrieval: lcm_grep, lcm_describe, and lcm_expand.",
    "- Do NOT use shell, exec, read-file, filesystem, git, sqlite, database, rg/grep/find, web, browser, or network tools.",
    "- If the Lossless recall tools are unavailable or would not finish quickly, do NOT fall back to shell or file inspection. Instead, synthesize the evidence from the embedded <active_summary_context> and state that limitation in confidenceNotes.",
    "",
    ...
    "4. Use lcm_expand directly on only the highest-value summary IDs to recover key details. For target briefs <= 1500 tokens, expand at most 3 summaries unless the active context is clearly insufficient.",
    "5. You are in delegated context, so do NOT call lcm_expand_query.",
    "6. Record synthesis-relevant uncertainty when you only inferred something from summaries or from the embedded active context.",
    "",
    ...
    "- Current Working State Evidence: active branches, files, tests, commands, blockers, and next actions only when present in summary/expansion evidence.",
...
function buildRefocusEvidenceTask(params: {
  ...
  return [
    ...
    "- Include concrete new decisions, file paths, command names, issue IDs, constraints, test results, deployment details, and next actions when those details appear in delta summaries or direct expansions.",
    "- Identify obsolete baseline details if the delta contradicts or supersedes them.",
    "- Keep this phase bounded. For lightweight targets, a concise delta evidence dossier is better than an exhaustive filesystem or repository audit.",
    "",
    "Tool boundary:",
    "- Use only Lossless recall tools for retrieval: lcm_grep, lcm_describe, and lcm_expand.",
    "- Do NOT use shell, exec, read-file, filesystem, git, sqlite, database, rg/grep/find, web, browser, or network tools.",
    "- If the Lossless recall tools are unavailable or would not finish quickly, do NOT fall back to shell or file inspection. Instead, synthesize the evidence from the embedded <delta_summary_context> and existing focus brief, and state that limitation in confidenceNotes.",
    "",
    "Required recall workflow:",
    ...
    "3. Use lcm_expand directly on only the highest-value delta summary IDs to recover details. For target briefs <= 1500 tokens, expand at most 3 summaries unless the delta context is clearly insufficient.",
    "4. You are in delegated context, so do NOT call lcm_expand_query.",
    "5. Separate genuinely new relevant delta from repeated baseline material.",
    "",
    "Return ONLY JSON with this shape:",
    "{",
